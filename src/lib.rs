...
top-level module. The top-level module also contains types for building
regular expressions (`Regex`) or fuzzy queries (`Levenshtein`), which can be
used to efficiently query a set or a map.
...
let map = try!(Map::from_file_path("map.fst"));
